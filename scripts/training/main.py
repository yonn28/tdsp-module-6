...
# +
def linear_regression_objective(trial):
    with mlflow.start_run(experiment_id=exp, run_name='linear_regression'):
        # Loggear el tipo de modelo como un parámetro
        ...
        # Loggear los hiperparámetros sugeridos por Optuna
...
def neural_network_objective(trial):
    with mlflow.start_run(experiment_id=exp, run_name="neural_network"):
        # Loggear el tipo de modelo como un parámetro
        mlflow.log_param("model_type", "neural_network")

        input_layer = tf.keras.layers.Input(shape=X_train.shape[1])
        x = tf.keras.layers.Dense(units=trial.suggest_int("units", 2, 50, log=True), activation="relu")(input_layer)
        ...
        history_deep = model_deep.fit(x=X_train, y=y_train, epochs=trial.suggest_int("epochs", 5, 50), batch_size=trial.suggest_int("batch_size", 8, 128, log=True))

        mlflow.keras.log_model(model_deep, "model")
...
def optimize_model(trial, model_type):
    if model_type == 'linear_regression':
        return linear_regression_objective(trial)
    elif model_type == 'neural_network':
        return neural_network_objective(trial)
    else:
        raise ValueError("Model type not supported")


def objective(trial):
    model_type = trial.suggest_categorical("model_type", ["linear_regression", "neural_network"])

    return optimize_model(trial, model_type)
